...
# Advisor Discussion with Codex CLI

Start a structured discussion with Codex CLI as an independent technical advisor about: $ARGUMENTS

## Your Role
You are Claude Code engaging in a structured discussion with Codex CLI as an independent technical advisor.
...
### 2. Execute Codex CLI
Use this exact command to call Codex CLI:
```bash
codex exec "[structured prompt]" --skip-git-repo-check
```

Send a structured prompt including:
- The topic being discussed: $ARGUMENTS
- Your (Claude Code's) position/suggestion/code
- Request for Codex CLI's analysis and opinion
- Reference to AGENTS.md for Codex CLI's role definition
...
- ✅ Respect Codex CLI's role as an independent advisor
...
This executes Codex CLI non-interactively without requiring a git repository.

## Prompt Structure for Codex CLI

```
Role: You are an independent technical advisor (see AGENTS.md)

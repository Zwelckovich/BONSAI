...
- **HTTP Client**: httpx (replaces requests - async support, HTTP/2, modern API)
- **Web Framework**: FastAPI (replaces Flask - async, auto validation, OpenAPI docs)
- **Web Scraping**: requests_html2 (modern web scraping with JavaScript support)
- **CLI Tools**: typer (replaces argparse - type-safe, modern CLI framework)
- **Terminal UIs**: textual (BONSAI-styled terminal user interfaces)
- **Progress Bars**: tqdm (CLI progress indicators with BONSAI colors)
- **Logging**: loguru (replaces logging module - beautiful, structured logging with BONSAI colors)
- **Data Cleaning**: janitor (dataframe cleaning utilities, use with pandas then convert to polars)
- **Console Output**: rich (replaces print statements - beautiful terminal output, panels, tables)
- **File Operations**: pathlib (replaces os.path - cross-platform, object-oriented paths)
- **Process Execution**: sh + rich (replaces subprocess - pythonic shell commands with rich output formatting)
